...
export const updateAppointmentStatus = async (req, res) => {
  try {
    ...
    const mailOptions = {
      ...
      subject: `Viewing Appointment ${status.charAt(0).toUpperCase() + status.slice(1)} - GState`,
...
export const scheduleViewing = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL,
      to: req.user.email,
      subject: "Viewing Scheduled - GState",
...
export const cancelAppointment = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL,
      to: appointment.userId.email,
      subject: 'Appointment Cancelled - GState',
      html: `
...
export const updateAppointmentMeetingLink = async (req, res) => {
  try {
    ...
    const mailOptions = {
      from: process.env.EMAIL,
      to: appointment.userId.email,
      subject: "Meeting Link Updated - GState",
      html: `
